...
%clear; close all;
msRun = importdata('E:\WaveBotData\waveBotCtrlMdl_2023_10_02_05_14_49sys.mat');
...
for it = [3,4]%4]
    ...
    % time domain
    u(1,k,:) = msRun.heaveSignals.heaveForceI_N(runSidx(k):runSidx(k) + N_per.*T_nom/dt-1);
    ...
    [~,~,~,comp1SpecU] = ampSpectra(msRun.refSignals.heaveRefRaw_N(runSidx(k):runSidx(k) + N_per.*T_nom/dt-1),[dt; 2*dt]);
...
figure; clf; 
subplot(2,2,1)
semilogx(f_vec2(f1:f2),squeeze(real(Y(1,1,f1:f2))),'-b');
hold on; grid on;
...
ylabel('Re(Y(1,1))')
subplot(2,2,2)
semilogx(f_vec2(f1:f2),squeeze(abs(Y(1,1,f1:f2))),'-b');
hold on; grid on;
...
ylabel('abs(Y(1,1))')
subplot(2,2,3)
semilogx(f_vec2(f1:f2),squeeze(imag(Y(1,1,f1:f2))),'-b');
hold on; grid on;
...
xlabel('Freq (Hz)')
subplot(2,2,4)
semilogx(f_vec2(f1:f2),squeeze(angle(Y(1,1,f1:f2))),'-b');
hold on; grid on;
...
legend('Y_I','Y_{ref}','Y_{TRS}','Y_{LCB}','Ytf_I','Ytf_{ref}','Ytf_{TRS}','Ytf_{LCB}',...
