...
out_var = co_optimize_PTO(Fe, Zi, PTO_cfg, w);
...
[P_tot, P_o, Z_L, den_P] = co_optimize_oneDOF_PTO_power(y, PTO_cfg.PTO_param_mask, PTO_param, w, Zi, Fe);
...
Z_L = Load_impedance(x(n_PTO_var+1:end), Z_PTO, w);
...
function ZL = Load_impedance(x, Z_PTO, w)
...
% function Z_mat = PTO_Impedance(PTO_param, w)
% 
% Nfreq = length(w);
% 
